...
const DEVICES = {
    ...
    'custom': { width: 480, height: 800, name: 'Custom', defaultPrefix: '' }
};
...
const DEFAULT_SETTINGS = {
    ...
    output: {
        ...
        filenamePrefix: null,  // null = use device default ([X3]/[X4]); '' or string = explicit
        dithering: true,
        ditherStrength: 0.7,
        negative: false
    },
...
function resolveSettings(settings) {
    ...
    // Default filename prefix from device when not explicitly set
    if (resolved.output.filenamePrefix == null) {
        const dev = DEVICES[settings.device];
        resolved.output.filenamePrefix = (dev && dev.defaultPrefix) ? dev.defaultPrefix : '';
    }
