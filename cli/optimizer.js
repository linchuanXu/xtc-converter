...
const { injectEpaperCss: injectEpaperCssFromLib } = require('./lib/epub-preprocess');
...
/**
 * Inject e-paper optimized CSS (uses lib so paragraph indent/spacing override book classes via !important)
 */
function injectEpaperCss(html, indent, spacing) {
    return injectEpaperCssFromLib(html, indent ?? 1, spacing ?? 20);
}
    ...
    for (const filePath of files) {
        ...
        if (/\.(html|xhtml|htm)$/i.test(filePath)) {
            ...
            if (options.injectCss) {
                html = injectEpaperCss(html, options.paraIndent, options.paraSpacing);
