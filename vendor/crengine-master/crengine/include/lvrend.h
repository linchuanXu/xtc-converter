...
typedef struct {
    ...
    /// Optional: pointer to std::vector<int>* to collect line bottom Y (in buffer coords) for each text line drawn. Used by WASM for underline positioning.
    void * line_baselines;
    /// Optional: pointer to std::vector<uint8_t>* to collect per-line "is heading" (1 = h1–h6). Same length as line_baselines. Used by WASM to avoid underlines in heading gaps.
    void * line_baselines_heading;
    /// Set by DrawDocument before drawing each final block; read by lvtextfm when pushing to line_baselines_heading. 1 = current block is h1–h6.
    uint8_t current_block_is_heading;
} draw_extra_info_t;
