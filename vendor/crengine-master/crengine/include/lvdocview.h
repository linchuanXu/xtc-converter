...
#define ORIGINAL_FILENAME_PATCH
    ...
    void * m_lineBaselinesCollector;  /// optional: std::vector<int>* to collect line bottom Y per page draw
    void * m_lineBaselinesHeadingCollector;  /// optional: std::vector<uint8_t>* to collect per-line is-heading (1 = h1–h6), same length as m_lineBaselinesCollector
    bool m_showCover;
...
public:
    ...
    /// set optional collector for line bottom Y coords (std::vector<int>*). When set, each drawn text line's bottom Y is appended. Used by WASM for underline positioning. Caller clears vector before Draw if needed.
    void setLineBaselinesCollector( void * collector ) { m_lineBaselinesCollector = collector; }
    /// set optional collector for per-line heading flag (std::vector<uint8_t>*). When set, 0 or 1 is appended per line (1 = h1–h6). Same length as line_baselines. Caller clears before Draw if needed.
    void setLineBaselinesHeadingCollector( void * collector ) { m_lineBaselinesHeadingCollector = collector; }
    /// get info line font
