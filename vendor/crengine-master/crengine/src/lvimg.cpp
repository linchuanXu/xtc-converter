...
#else
                    lUInt8 *rgbBuf = new lUInt8 [ cinfo.output_width * 3 ];
                    lUInt8 *rgbPtr = rgbBuf;
                    (void) jpeg_read_scanlines(&cinfo, &rgbPtr, 1);
                    for (unsigned int x = 0; x < cinfo.output_width; x++) {
                        buffer[x*4 + 0] = rgbBuf[x*3 + 2]; // B
                        buffer[x*4 + 1] = rgbBuf[x*3 + 1]; // G
                        buffer[x*4 + 2] = rgbBuf[x*3 + 0]; // R
                        buffer[x*4 + 3] = 0x00;             // A
                    }
                    delete[] rgbBuf;
#endif
