...
void LFormattedText::Draw( LVDrawBuf * buf, int x, int y, ldomMarkedRangeList * marks, ldomMarkedRangeList *bookmarks )
{
    ...
    for (i=0; i<m_pbuffer->frmlinecount; i++)
    {
        ...
        {
            // Report line bottom Y for frontend underline positioning (e.g. WASM getLineBaselines).
            // Skip image-only lines so underlines are not drawn under pictures.
            bool line_has_text = false;
            for (int jj = 0; jj < frmline->word_count; jj++) {
                if ( !(frmline->words[jj].flags & LTEXT_WORD_IS_IMAGE) ) {
                    line_has_text = true;
                    break;
                }
            }
            if ( line_has_text ) {
                if ( draw_extra_info && draw_extra_info->line_baselines )
                    reinterpret_cast<std::vector<int>*>(draw_extra_info->line_baselines)->push_back( y + frmline->y + frmline->height );
                if ( draw_extra_info && draw_extra_info->line_baselines_heading )
                    reinterpret_cast<std::vector<uint8_t>*>(draw_extra_info->line_baselines_heading)->push_back( draw_extra_info->current_block_is_heading );
            }
