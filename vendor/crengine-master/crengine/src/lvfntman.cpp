...
public:
    ...
    bool loadFromFile( const char * fname, int index, int size, css_font_family_t fontFamily,
        ...
        if (error != FT_Err_Ok) {
            fprintf(stderr, "CRE: loadFromFile: FT_New_Face failed: %#x path='%s'\n", error, _fileName.c_str());
            return false;
        }
...
public:
    ...
    */
    ...
    virtual bool RegisterFont( lString8 name )
    {
        ...
        printf("CRE: RegisterFont name='%s' path='%s'\n", name.c_str(), fname.c_str());
        #if (DEBUG_FONT_MAN==1)
            if ( _log ) {
        ...
        // Verify file accessibility before calling FreeType
        {
            FILE *check = fopen(fname.c_str(), "rb");
            if (!check) {
                printf("CRE: RegisterFont: fopen('%s','rb') failed - file not accessible\n", fname.c_str());
            } else {
                fseek(check, 0, SEEK_END);
                long sz = ftell(check);
                fclose(check);
                printf("CRE: RegisterFont: file '%s' accessible, size=%ld\n", fname.c_str(), sz);
            }
        }
