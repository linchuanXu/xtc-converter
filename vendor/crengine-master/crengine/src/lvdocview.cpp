...
LVDocView::LVDocView(int bitsPerPixel, bool noDefaultDocument) :
	m_bitsPerPixel(bitsPerPixel), m_dx(400), m_dy(200), _pos(0), _page(0),
			...
			m_pageHeaderInfo(PGHDR_PAGE_NUMBER
					...
					| PGHDR_TITLE), m_lineBaselinesCollector(nullptr), m_lineBaselinesHeadingCollector(nullptr), m_showCover(true)
...
LVImageSourceRef LVDocView::getCoverPageImage() {
	// EPUB/other formats: use DOC_PROP_COVER_FILE set by loader (e.g. epubfmt.cpp)
	...
	// FB2: look up coverpage element in document
	lUInt16 path[] = { el_FictionBook, el_description, el_title_info, el_coverpage, 0 };
	//lUInt16 path[] = { el_FictionBook, el_description, el_title_info, el_coverpage, el_image, 0 };
	ldomNode * cover_el = 0;
	ldomNode * rootNode = m_doc->getRootNode();
...
void LVDocView::drawPageTo(LVDrawBuf * drawbuf, LVRendPageInfo & page,
	...
	if (m_lineBaselinesCollector)
		reinterpret_cast<std::vector<int>*>(m_lineBaselinesCollector)->clear();
	if (m_lineBaselinesHeadingCollector)
		reinterpret_cast<std::vector<uint8_t>*>(m_lineBaselinesHeadingCollector)->clear();
	draw_extra_info.line_baselines = m_lineBaselinesCollector;
	draw_extra_info.line_baselines_heading = m_lineBaselinesHeadingCollector;
	draw_extra_info.current_block_is_heading = 0;
