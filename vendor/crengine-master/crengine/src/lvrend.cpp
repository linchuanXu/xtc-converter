    ...
    if (!style->background_image.empty()) {
        ...
        if (!img.isNull()) {
            ...
            */
            ...
            // Draw as background image so downstream consumers can classify this region.
            drawbuf.setImageDrawHint(1);
            drawbuf.Draw(transformed, x0+doc_x+draw_x, y0+doc_y+draw_y, transform_w, transform_h);
            drawbuf.setImageDrawHint(0);
            if (clip_to_target) {
        ...
        case erm_block:
            {
                ...
                if ( style->display == css_d_list_item_block && !renderAsListStylePositionInside(style, is_rtl) && !isHidden ) {
                    ...
                    */
                    ...
                    {
                        draw_extra_info_t * draw_extra = (draw_extra_info_t*)drawbuf.GetDrawExtraInfo();
                        if ( draw_extra ) draw_extra->current_block_is_heading = 0;  // list marker is not a heading
                    }
                    if ( is_rtl ) {
        ...
        case erm_final:
            {
                ...
                fmt.push();
                {
                    ...
                    ldomMarkedRangeList *nbookmarks = NULL;
                    {
                        draw_extra_info_t * draw_extra = (draw_extra_info_t*)drawbuf.GetDrawExtraInfo();
                        if ( draw_extra )
                            draw_extra->current_block_is_heading = ( enode->getNodeId() >= el_h1 && enode->getNodeId() <= el_h6 ) ? 1 : 0;
                    }
