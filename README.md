...
See [docs/xtc-format-spec.md](docs/xtc-format-spec.md) for technical details.

## Self-Hosting

### Font Hosting

The web app loads fonts (TTF files) at runtime from a configurable CDN base URL. You need to host these fonts yourself or configure a CDN:

1. **Self-host**: Place TTF files under `web/system/fonts/` and run `npm run dev` (the dev server serves them from `/system/fonts/`)
2. **Remote CDN**: Set `FONT_CDN_BASE` in `web/app.fonts.js` to your CDN URL, and set `FONT_ORIGIN` env var when running the dev server

All fonts used are open-source (Google Fonts / OFL licensed). Many are available via `https://raw.githubusercontent.com/google/fonts/main/`.

### Option A: Run from source (dev/debug)

Clone and serve the `web/` directory:

```bash
# Recommended: dev server with font proxy (default http://localhost:8009)
npm run dev

# With custom font CDN:
FONT_ORIGIN=https://your-cdn.example.com npm run dev

# Or use any static server (fonts from /system/fonts/ or configure FONT_CDN_BASE)
cd web && python -m http.server 8000
```

### Option B: Build then deploy (minified/obfuscated, recommended)

**Local build (option 1)**: Bundle and minify JS with Terser, output to `dist/`:
...
**Docker (option 2)**: Builds automatically inside container, final image only contains `dist/` without source:

```bash
docker build -t epub-to-xtc-web .
docker run -p 8000:8000 epub-to-xtc-web
```

Then open http://localhost:8000.

### Option C: Custom CREngine WASM (for deep font crash issues)

When all CJK fonts trigger WASM crashes at `getPageCount()`, rebuild `crengine.wasm` from source:

```bash
# Requires external CREngine source directory
CRENGINE_SRC_DIR=/path/to/crengine/source npm run build:crengine
npm run build
```

See [docs/crengine-wasm-build.md](docs/crengine-wasm-build.md) for details.
